"""Session-scoped log (.log) creation, inspection, and editing tools.

Like the other source editors, this module is a thin per-format adapter:
...
def build_log_edit_tools(
    settings: Settings,
    *,
    session_root: Path | None = None,
    thread_id: str = "",
) -> list[BaseTool]:
    """Create log tools only when enabled and safely scoped to a chat."""

    return build_source_edit_tools(
        settings, LOG_CONFIG, session_root=session_root, thread_id=thread_id
    )
