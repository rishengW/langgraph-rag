"""Session-scoped TOML (.toml) creation, inspection, and editing tools.

Mirrors the JSON tools: the inspector lists dotted paths and the editor
applies path-based operations validated against the document before
anything is written. Parsing uses the stdlib ``tomllib`` (Python 3.11+);
serialization uses a small built-in writer because no TOML writing
library is in the dependency stack. The uploaded source is never modified.
"""
...
TOML_MIME_TYPE = "application/toml"
_MAX_VALUE_CHARS = 100_000
_MAX_INSPECT_ENTRIES = 1_000
_MAX_SCALAR_CHARS = 200

TOML_CONFIG = SourceEditConfig(
    label="TOML",
    suffixes=(".toml",),
    creation_suffix=".toml",
    tool_prefix="toml",
    mime_type=TOML_MIME_TYPE,
    flag_name="toml_edit_enabled",
    temp_prefix=".toml_write-",
)


class TomlEditError(Exception):
    """Raised when a TOML file cannot be inspected or edited safely."""
...
class TomlEditOperation(BaseModel):
    ...
    def _check_fields(self) -> TomlEditOperation:
        try:
...
class TomlInspectInput(BaseModel):
    """Input schema for the TOML inspector."""

    path: str = Field(
        ...,
        min_length=1,
        description=(
            "Path to a .toml file uploaded to this chat session, exactly as "
...
class TomlCreateInput(BaseModel):
    """Input schema for creating a TOML file."""
    ...
    filename: str = Field(
        ...
        max_length=200,
        description=(
            "Bare filename for the new TOML file. A .toml suffix is added "
    ...
    content: str = Field(
        ...,
        description=(
            "Complete UTF-8 TOML text to write. It must parse as valid "
            "TOML; formatting is preserved as provided."
        ),
    )


class TomlEditInput(BaseModel):
    """Input schema for the TOML editor."""

    path: str = Field(
        ...,
        min_length=1,
        description=(
            "Path to a .toml file uploaded to this chat session. The source is never modified."
        ),
    )
    operations: list[TomlEditOperation] = Field(
        ...,
        min_length=1,
    ...
    output_name: str | None = Field(
        ...
        description=(
            ...
            "'<original>.edited.toml'; a .toml suffix is enforced."
        ),
    )


def build_toml_edit_tools(
    settings: Settings,
    *,
    session_root: Path | None = None,
    thread_id: str = "",
) -> list[BaseTool]:
    """Create TOML tools only when enabled and safely scoped to a chat."""

    if not (settings.file_read_enabled and settings.toml_edit_enabled):
        return []
    ...
    def _run_edit(
        path: str,
        operations: list[TomlEditOperation],
        output_name: str | None = None,
    ) -> tuple[str, dict[str, object] | None]:
        result = edit_toml_file(
            path,
    ...
    create_tool = StructuredTool.from_function(
        func=_run_create,
        name="create_toml_file",
        description=(
            "Create a new UTF-8 .toml file in the current chat session and "
            "return it as a download. The content must parse as valid TOML. "
            "Existing files are never overwritten; a numbered filename is "
            "chosen on collision. Use only when the user explicitly asks to "
            "create a TOML file."
        ),
        args_schema=TomlCreateInput,
        response_format="content_and_artifact",
    )
    inspect_tool = StructuredTool.from_function(
        func=_run_inspect,
        name="inspect_toml_file",
        description=(
            "List the documented paths ($, $.key, $.items[0], ...) and values "
            "of a .toml file uploaded to this chat session. Call this before "
            "edit_toml_file to obtain the exact paths and current values "
            "required by edit operations."
        ),
        args_schema=TomlInspectInput,
    )
    edit_tool = StructuredTool.from_function(
        ...
        description=(
            ...
            "to an uploaded .toml file and create a new downloadable copy "
            "without changing the original. Call inspect_toml_file first; "
            "pass expected_value for existing paths or expected_missing=true "
            "to create a path. The copy is re-serialized as standard TOML "
            "(comments and original formatting are not preserved). Use only "
            "when the user explicitly asks to change the file."
        ),
        args_schema=TomlEditInput,
        response_format="content_and_artifact",
    )
...
def create_toml_file(
    ...
    *,
    ...
    """Create a session-scoped UTF-8 TOML file and downloadable artifact."""

    output_name = "(unresolved)"
    try:
        if not _THREAD_ID_PATTERN.fullmatch(thread_id):
            raise TomlEditError("a valid chat thread is required to create a TOML file.")
        directory = _prepare_session_directory(
            session_root=session_root,
            file_root=file_root,
        )
        stem, suffix = _creation_stem(TOML_CONFIG, filename)
    ...
    except (OSError, UnicodeError, SourceEditError, TomlEditError) as exc:
        logger.info(
            "toml_create failed: thread=%s output=%s reason=%s",
            thread_id or "(none)",
            output_name,
            type(exc).__name__,
        )
        return SourceEditResult(content=f"Could not create TOML file: {exc}")

    logger.info(
        "toml_create succeeded: thread=%s output=%s bytes=%d",
        thread_id,
        published.name,
        size_bytes,
    )
...
def inspect_toml_file(
    ...
    *,
    ...
    try:
        ...
        return f"Could not inspect TOML file: {exc}"
    except TomlEditError as exc:
        return f"Could not inspect TOML file: {exc}"
...
def edit_toml_file(
    ...
    *,
    ...
    try:
        ...
        logger.info(
            "toml_edit failed: thread=%s source=%s operations=%d reason=%s",
            thread_id or "(none)",
            source_name,
            len(operations),
            type(exc).__name__,
        )
        return SourceEditResult(content=f"Could not edit TOML file: {exc}")
    except TomlEditError as exc:
        logger.info(
            "toml_edit failed: thread=%s source=%s operations=%d reason=%s",
            thread_id or "(none)",
    ...
    size_bytes = published.stat().st_size
    logger.info(
        "toml_edit succeeded: thread=%s source=%s output=%s operations=%d bytes=%d",
        thread_id or "(none)",
        source_name,
        published.name,
        len(operations),
...
def _parse_document_text(document: _SourceDocument, name: str) -> Any:
    """Parse the decoded document text, raising TomlEditError on bad TOML."""

    text = document.newline_text.join(document.lines)
    try:
        return tomllib.loads(text)
    except (ValueError, tomllib.TOMLDecodeError) as exc:
        raise TomlEditError(f"{name!r} is not valid TOML: {exc}") from exc
...
def _document_from_text(
    ...
    *,
    ...
    """Rebuild a line document from serialized TOML text."""
...
def _parse_path(path: str) -> list[tuple[str, tuple[int, ...]]]:
    ...
    for raw in text.split(".") if text else []:
        ...
        if not match:
            raise TomlEditError(f"invalid path segment {raw!r} in {path!r}.")
        name, brackets = match.groups()
        indexes = tuple(int(value) for value in re.findall(r"\[(\d+)\]", brackets))
        if not name and not indexes:
            raise TomlEditError(f"invalid empty path segment in {path!r}.")
...
def _step(node: Any, kind: str, key: str | int, where: str) -> Any:
    if isinstance(key, str):
        if not isinstance(node, dict) or key not in node:
            raise TomlEditError(f"path {where} does not exist.")
        return node[key]
    if not isinstance(node, list) or key >= len(node):
        raise TomlEditError(f"path {where} does not exist (index out of range).")
    return node[key]
...
def _json_loads(raw: str, where: str) -> Any:
    ...
    try:
        ...
        raise TomlEditError(f"could not decode a JSON value at {where}: {exc}") from exc


def _check_operation(document: Any, operation: TomlEditOperation) -> str | None:
        ...
        if operation.action == "set_value":
            ...
            if kind == "key":
                if not isinstance(parent, dict):
                    return f"path {where} is not inside a table."
                if key in parent:
...
def _apply_operation(document: Any, operation: TomlEditOperation) -> Any:
    """Apply one already-validated operation and return the (new) root."""

    problem = _check_operation(document, operation)
    if problem:
        raise TomlEditError(f"{operation.action} at {operation.path or '$'}: {problem}")
...
def _walk_paths(node: Any, path: str, out: list[str]) -> None:
    ...
    if isinstance(node, dict):
        out.append(f"{here} = table ({len(node)} keys)")
        ...
        encoded = json.dumps(node, ensure_ascii=False, default=str)
...
def _format_key(key: str) -> str:
    return key if _BARE_KEY.fullmatch(key) else '"' + key.replace("\\", "\\\\").replace('"', '\\"') + '"'
...
def _format_scalar(value: Any) -> str:
    ...
    raise TomlEditError(f"unsupported TOML value type: {type(value).__name__}")
...
def _dump_table(
    ...
    """Emit one table: scalars/arrays first, then sub-tables, then table arrays."""
    ...
    for key in simple:
        ...
        if isinstance(value, list):
            # Inline array of scalars or mixed values (not a table array).
            items = ", ".join(_format_scalar(item) for item in value)
            lines.append(f"{_format_key(key)} = [{items}]")
        else:
            lines.append(f"{_format_key(key)} = {_format_scalar(value)}")
    if simple and (sub_tables or table_arrays) and prefix:
        lines.append("")

    for key in sub_tables:
        header = f"{prefix}{_format_key(key)}"
        lines.append(f"[{header}]")
        _dump_table(table[key], f"{header}.", lines)
        lines.append("")
    ...
    for key in table_arrays:
        ...
        for item in table[key]:
            lines.append(f"[[{header}]]")
            _dump_table(item, f"{header}.", lines)
            lines.append("")

    # Empty tables still need an explicit header; non-empty ones were
    # emitted above via sub_tables.


def dump_toml(data: dict[str, Any]) -> str:
    """Serialize a parsed TOML document back to standard TOML text."""

    if not isinstance(data, dict):
        raise TomlEditError("the TOML document root must be a table.")
    lines: list[str] = []
    _dump_table(data, "", lines)
    text = "\n".join(lines)
    # Collapse runs of blank lines left by section emission.
    text = re.sub(r"\n{3,}", "\n\n", text)
