"""Session-scoped XML (.xml) creation, inspection, and editing tools.

Like the JSON/YAML editors, XML is addressed structurally: the inspector
lists dotted paths and the editor applies path-based operations validated
against the document before anything is written. Parsing and serialization
use the stdlib ``xml.etree.ElementTree``. Each element maps to a dict whose
keys are child tags (a list when a tag repeats), ``@name`` for attributes,
and ``#text`` for the element's own text. Re-serialization normalizes
formatting (comments, processing instructions, and the XML declaration
beyond the root tag are not preserved); the uploaded source is never
modified.
"""
...
XML_CONFIG = SourceEditConfig(
    label="XML",
    suffixes=(".xml",),
    creation_suffix=".xml",
    tool_prefix="xml",
    mime_type=XML_MIME_TYPE,
    flag_name="xml_edit_enabled",
    temp_prefix=".xml_write-",
)


class XmlEditError(Exception):
    """Raised when an XML file cannot be inspected or edited safely."""
...
class XmlEditOperation(BaseModel):
    ...
    action: Literal["set_value", "delete_key", "append_to_array"] = Field(
        ..., description="The kind of XML edit to apply."
    )
    path: str = Field(
        default="",
        max_length=500,
    ...
    def _check_fields(self) -> XmlEditOperation:
        try:
            _parse_path(self.path)
        except XmlEditError as exc:
...
class XmlInspectInput(BaseModel):
    """Input schema for the XML inspector."""

    path: str = Field(
        ...,
        min_length=1,
        description=(
            "Path to a .xml file uploaded to this chat session, exactly as "
...
class XmlCreateInput(BaseModel):
    """Input schema for creating an XML file."""
    ...
    filename: str = Field(
        ...
        max_length=200,
        description=(
            "Bare filename for the new XML file. A .xml suffix is added "
    ...
    content: str = Field(
        ...,
        description=(
            "Complete UTF-8 XML text to write. It must parse as "
            "well-formed XML; formatting is preserved as provided."
        ),
    )


class XmlEditInput(BaseModel):
    """Input schema for the XML editor."""

    path: str = Field(
        ...,
        min_length=1,
        description=(
            "Path to a .xml file uploaded to this chat session. The source is never modified."
        ),
    )
    operations: list[XmlEditOperation] = Field(
        ...,
        min_length=1,
    ...
    output_name: str | None = Field(
        ...
        description=(
            ...
            "'<original>.edited.xml'; a .xml suffix is enforced."
        ),
    )


def build_xml_edit_tools(
    settings: Settings,
    *,
    session_root: Path | None = None,
    thread_id: str = "",
) -> list[BaseTool]:
    """Create XML tools only when enabled and safely scoped to a chat."""

    if not (settings.file_read_enabled and settings.xml_edit_enabled):
        return []
    ...
    def _run_edit(
        path: str,
        operations: list[XmlEditOperation],
        output_name: str | None = None,
    ) -> tuple[str, dict[str, object] | None]:
        result = edit_xml_file(
            path,
    ...
    create_tool = StructuredTool.from_function(
        ...
        description=(
            "Create a new UTF-8 .xml file in the current chat session and "
            "return it as a download. The content must be well-formed XML. "
            "Existing files are never overwritten; a numbered filename is "
            "chosen on collision. Use only when the user explicitly asks to "
            "create an XML file."
        ),
        args_schema=XmlCreateInput,
        response_format="content_and_artifact",
    )
    inspect_tool = StructuredTool.from_function(
        func=_run_inspect,
        name="inspect_xml_file",
        description=(
            "List the documented paths ($, $.key, $.items[0], ...) and "
            "values of a .xml file uploaded to this chat session. Child "
            "elements appear by tag name, attributes as '@name' keys, and "
            "an element's own text as '#text'. Call this before "
            "edit_xml_file to obtain the exact paths and current values "
            "required by edit operations."
        ),
        args_schema=XmlInspectInput,
    )
    edit_tool = StructuredTool.from_function(
        func=_run_edit,
        name="edit_xml_file",
        description=(
            "Apply path-based edits (set_value, delete_key, "
            "append_to_array) to an uploaded .xml file and create a new "
            "downloadable copy without changing the original. Call "
            "inspect_xml_file first; pass expected_value for existing "
            "paths or expected_missing=true to create a path. The copy is "
            "re-serialized as standard XML (comments and the original "
            "formatting are not preserved). Use only when the user "
            "explicitly asks to change the file."
        ),
        args_schema=XmlEditInput,
        response_format="content_and_artifact",
    )
...
def create_xml_file(
    ...
    *,
    ...
    """Create a session-scoped UTF-8 XML file and downloadable artifact."""

    output_name = "(unresolved)"
    try:
        if not _THREAD_ID_PATTERN.fullmatch(thread_id):
            raise XmlEditError("a valid chat thread is required to create an XML file.")
        directory = _prepare_session_directory(
            session_root=session_root,
            file_root=file_root,
        )
        stem, suffix = _creation_stem(XML_CONFIG, filename)
    ...
    except (OSError, UnicodeError, SourceEditError, XmlEditError) as exc:
        logger.info(
            "xml_create failed: thread=%s output=%s reason=%s",
            thread_id or "(none)",
            output_name,
            type(exc).__name__,
        )
        return SourceEditResult(content=f"Could not create XML file: {exc}")

    logger.info(
        "xml_create succeeded: thread=%s output=%s bytes=%d",
        thread_id,
        published.name,
        size_bytes,
    )
...
def inspect_xml_file(
    ...
    *,
    ...
    try:
        ...
        return f"Could not inspect XML file: {exc}"
    except XmlEditError as exc:
        return f"Could not inspect XML file: {exc}"
...
def edit_xml_file(
    ...
    *,
    ...
    try:
        ...
        logger.info(
            "xml_edit failed: thread=%s source=%s operations=%d reason=%s",
            thread_id or "(none)",
            source_name,
            len(operations),
            type(exc).__name__,
        )
        return SourceEditResult(content=f"Could not edit XML file: {exc}")
    except XmlEditError as exc:
        logger.info(
            "xml_edit failed: thread=%s source=%s operations=%d reason=%s",
            thread_id or "(none)",
    ...
    size_bytes = published.stat().st_size
    logger.info(
        "xml_edit succeeded: thread=%s source=%s output=%s operations=%d bytes=%d",
        thread_id or "(none)",
        source_name,
        published.name,
        len(operations),
...
def _parse_document_text(document: _SourceDocument, name: str) -> Any:
    """Parse the decoded document text into the element-dict model."""

    text = document.newline_text.join(document.lines)
    try:
        root = ET.fromstring(text)
    except ET.ParseError as exc:
        raise XmlEditError(f"{name!r} is not well-formed XML: {exc}") from exc
    # Wrap under the root tag so the document is one dict keyed by it.
...
def _document_from_text(
    ...
    *,
    ...
    """Rebuild a line document from serialized XML text."""
...
def _parse_path(path: str) -> list[tuple[str, tuple[int, ...]]]:
    ...
    for raw in text.split(".") if text else []:
        ...
        if not match:
            raise XmlEditError(f"invalid path segment {raw!r} in {path!r}.")
        name, brackets = match.groups()
        indexes = tuple(int(value) for value in re.findall(r"\[(\d+)\]", brackets))
        if not name and not indexes:
            raise XmlEditError(f"invalid empty path segment in {path!r}.")
...
def _step(node: Any, kind: str, key: str | int, where: str) -> Any:
    if isinstance(key, str):
        if not isinstance(node, dict) or key not in node:
            raise XmlEditError(f"path {where} does not exist.")
        return node[key]
    if not isinstance(node, list) or key >= len(node):
        raise XmlEditError(f"path {where} does not exist (index out of range).")
    return node[key]
...
def _json_loads(raw: str, where: str) -> Any:
    try:
        return json.loads(raw)
    except ValueError as exc:
        raise XmlEditError(f"could not decode a JSON value at {where}: {exc}") from exc


def _check_operation(document: Any, operation: XmlEditOperation) -> str | None:
    ...
    try:
        ...
        if operation.action == "set_value":
            if not steps:
                ...
                expected = _json_loads(operation.expected_value, where)
                return (
                    None
            ...
            if kind == "key":
                if not isinstance(parent, dict):
                    return f"path {where} is not inside an element."
                if key in parent:
                    if operation.expected_missing:
                        return f"path {where} already exists but expected_missing was set."
                    assert operation.expected_value is not None
                    if parent[key] != _json_loads(operation.expected_value, where):
...
def _apply_operation(document: Any, operation: XmlEditOperation) -> Any:
    """Apply one already-validated operation and return the (new) root."""

    problem = _check_operation(document, operation)
    if problem:
        raise XmlEditError(f"{operation.action} at {operation.path or '$'}: {problem}")

    steps = _steps(operation.path)
    where = operation.path or "$"
    value = _json_loads(operation.value, where) if operation.value is not None else None
    if operation.action == "set_value":
        if not steps:
...
def _walk_paths(node: Any, path: str, out: list[str]) -> None:
    ...
    if isinstance(node, dict):
        out.append(f"{here} = element ({len(node)} members)")
...
def element_to_dict(element: ET.Element) -> dict[str, Any]:
    """Convert one element into the dict model used by path-based editing."""

    node: dict[str, Any] = {}
    for name, value in element.attrib.items():
        node[f"{_ATTR_PREFIX}{name}"] = value

    children: dict[str, list[Any]] = {}
    for child in element:
        converted = element_to_dict(child)
    ...
    if text:
        node[_TEXT_KEY] = text
    # An element with no attributes, children, or text is an empty dict,
    # which serializes back as a self-closing tag.
    return node


def _looks_like_element(value: Any) -> bool:
    """True when a decoded value should be treated as an element dict."""

    return isinstance(value, dict) and (
        any(key == _TEXT_KEY or key.startswith(_ATTR_PREFIX) for key in value)
        or all(not key.startswith(("$", _TEXT_KEY)) for key in value)
        or not value
    )


def dict_to_element(tag: str, node: dict[str, Any]) -> ET.Element:
    """Convert the dict model back into one element (recursively)."""

    element = ET.Element(tag)
    for key, value in node.items():
        if key.startswith(_ATTR_PREFIX):
            element.set(key[len(_ATTR_PREFIX):], _stringify(value))
        elif key == _TEXT_KEY:
            element.text = _stringify(value)
    for key, value in node.items():
        if key.startswith(_ATTR_PREFIX) or key == _TEXT_KEY:
            continue
        if isinstance(value, list):
            for item in value:
                _append_child(element, key, item)
        else:
            _append_child(element, key, value)
    return element


def _append_child(parent: ET.Element, tag: str, value: Any) -> None:
    if isinstance(value, dict):
        parent.append(dict_to_element(tag, value))
    else:
...
def _stringify(value: Any) -> str:
    ...
    if isinstance(value, (dict, list)):
        raise XmlEditError(f"cannot place a {type(value).__name__} value as text or an attribute.")
    return str(value)


def dump_xml(data: dict[str, Any]) -> str:
    """Serialize the root element dict back to an XML document string."""

    if not isinstance(data, dict):
        raise XmlEditError("the XML document root must be an element.")
    root_tags = [key for key in data if not key.startswith(_ATTR_PREFIX) and key != _TEXT_KEY]
    if len(root_tags) != 1:
        raise XmlEditError(
            "the XML document must have exactly one root element; "
            f"found {len(root_tags)} candidate tags."
        )
    # Root attributes/text live beside the single root tag and are folded in.
    root_tag = root_tags[0]
    root_value = data[root_tag]
    if isinstance(root_value, list):
        raise XmlEditError("the XML document root cannot repeat.")
    merged = {key: value for key, value in data.items() if key != root_tag}
    if isinstance(root_value, dict):
        merged = {**merged, **root_value}
    root = dict_to_element(root_tag, merged)
    ET.indent(root)
