...
class XmlFileInput(BaseModel):
    """Input schema for the XML file reader."""

    path: str = Field(
        ...,
        min_length=1,
        description=(
            "Path to a .xml file, relative to the configured file-read "
...
def build_xml_file_tool(
    settings: Settings,
) -> BaseTool:
    """Create an XML file reader confined to the file-read root."""

    root = Path(settings.file_read_root)
    max_bytes = settings.file_read_max_bytes

    def _run_xml_file(path: str, max_chars: int = _MAX_CHARS) -> str:
...
def read_xml_file(
    ...
    *,
    ...
    try:
        ...
        return f"Could not read XML file: {exc}"

    try:
        from ._file_cache import PARSED_FILE_CACHE

        # XML shares the UTF-8 text parser key, so identical uploaded
        ...
        return f"Could not read XML file {resolved.name!r}: {exc}"
