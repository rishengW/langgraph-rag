...
class YamlFileInput(BaseModel):
    """Input schema for the YAML file reader."""

    path: str = Field(
        ...,
        min_length=1,
        description=(
            "Path to a .yaml or .yml file, relative to the configured "
...
def build_yaml_file_tool(
    settings: Settings,
) -> BaseTool:
    """Create a YAML file reader confined to the file-read root."""

    root = Path(settings.file_read_root)
    max_bytes = settings.file_read_max_bytes

    def _run_yaml_file(path: str, max_chars: int = _MAX_CHARS) -> str:
    ...
    return StructuredTool.from_function(
        ...
        name="read_yaml_file",
        description=(
            "Read the raw contents of a .yaml or .yml file from the local "
            "document directory (including files uploaded to the current "
            "chat session). Use when the user references a YAML file by name "
            "and wants its contents read, summarized, or searched. Returns "
            "the raw text (possibly truncated)."
        ),
...
def read_yaml_file(
    ...
    *,
    ...
    try:
        ...
        return f"Could not read YAML file: {exc}"

    try:
        from ._file_cache import PARSED_FILE_CACHE

        # YAML and plain text share a parser key, so identical uploaded
        ...
        return f"Could not read YAML file {resolved.name!r}: {exc}"
