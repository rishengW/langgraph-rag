...
def build_web_search_prompt(
    ...
    *,
    ...
    instructions = (
        ...
        "sources.\n\n"
        "Answer the question using the web sources below. If the sources "
        "genuinely do not contain the answer after semantic matching, say so "
        "plainly rather than guessing from prior knowledge.\n\n"
        "CITATIONS — Cite a source by writing its full URL in parentheses, for "
        "example (https://example.com/article). The sources below have no "
        "numbers, IDs, or line numbers, so never invent bracketed reference "
        "markers, source indexes, line ranges, or footnote symbols such as "
        "[1], \u3010199\u2020L91-L126\u3011, or [oaicite:0]. A citation that is "
        "not a URL copied from the source list is wrong.\n\n"
    )
