...
def test_stdio_boundary_propagates_and_restores_trusted_tool_principal():
    ...
    class PrincipalCapturingService:
        ...
        async def ask(self, request):
            self.principals.append(current_tool_principal())
            return RagAnswer(
    ...
    async def run():
        service = PrincipalCapturingService()
        server, _ = create_mcp_server(
            service=service,
            settings=MCPSettings(enabled=True, transport="stdio"),
        )
        async with Client(server) as client:
            result = await client.call_tool("rag_ask", {"question": "Who am I?"})
        assert result.is_error is False
        assert [(item.principal_id, item.tenant_id) for item in service.principals] == [
            ("local-process", None)
        ]
        assert current_tool_principal().principal_id == "anonymous"
